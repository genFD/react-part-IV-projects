...
3. Divide the applications features into **categories**
...
The first step is the business requirement, meaning **what does the business expect from the application?** :

- A very simple application where user can **order pizzas from a menu**
...
The step 2 and 3 are about dividing the application into pages and features categories.
...
From the first step we also understand that we can divide the **application into four four pages** :

1. **HOMEPAGE** where users can enter their name to place an order : `/`
2. **PIZZA MENU** where users can see the list of pizzas: `/menu`
3. **CART** where users can add or remove orders : `/cart`
4. **PLACE A NEW ORDER** `/order/new`
5. **LOOKING UP A NEW ORDER** `/order/:orderID`

**Step 3 and 4** :

We need to start thinking about state management and decide which technologies to use for this application. Based on our four features categories, we can divide the state into four slices(state domains) :
